...
export async function createProblemFiles({ workspaceRoot, question, langSlug, force = false }) {
  ...
  const problemDescription = problemMarkdown(question);
  ...
  await writeIfNeeded(path.join(problemDir, 'problem.md'), problemDescription, { force: true, files });
  await writeIfNeeded(path.join(problemDir, 'cases.txt'), `${question.sampleTestCase || ''}\n`, { force, files });
  await writeIfNeeded(path.join(problemDir, meta.solutionFile), solutionSource({
    problemDescription,
    code: snippet.code,
    langSlug,
  }), { force, files });
...
export function findProblemDir(workspaceRoot, slugOrId) {
  ...
  const candidates = readdirSyncCompat(problemsRoot, { withFileTypes: true })
    ...
    .map((entry) => problemCandidate(path.join(problemsRoot, entry.name)))
    .sort((left, right) => left.base.localeCompare(right.base));

  const normalized = normalizeProblemRef(slugOrId);
  ...
  if (isProblemIndex(normalized)) {
    const exactId = candidates.find((candidate) => (
      candidate.ids.some((id) => normalizeProblemIndex(id) === normalizeProblemIndex(normalized))
    ));
    return exactId?.path;
  }

  const partial = candidates.find((candidate) => candidate.slug.includes(normalized) || candidate.base.includes(normalized));
  return partial?.path;
}
...
function solutionSource({ problemDescription, code, langSlug }) {
  if (langSlug !== 'cpp') {
    return code;
  }

  return `${lineComment(problemDescription, '//')}\n\n${code}`;
}

function lineComment(text, marker) {
  return text
    .replace(/\r/g, '')
    .split('\n')
...
function problemCandidate(problemDir) {
  const base = path.basename(problemDir).toLowerCase();
  const meta = readProblemMetaSync(problemDir);
  const dirIndex = base.match(/^([^_]+)_/)?.[1];
  const fallbackSlug = base.replace(/^[^_]+_/, '');
  const slug = normalizeProblemRef(meta?.titleSlug || fallbackSlug);
  const ids = [
    dirIndex,
...
function readProblemMetaSync(problemDir) {
  try {
    return JSON.parse(readFileSync(path.join(problemDir, 'meta.json'), 'utf8'));
  } catch {
    return undefined;
  }
}
...
function isProblemIndex(value) {
  return /^\d+$/.test(String(value).trim());
}

function normalizeProblemIndex(value) {
  const normalized = String(value).trim();
  return isProblemIndex(normalized) ? String(Number.parseInt(normalized, 10)) : normalized.toLowerCase();
}
